...
void Laborator9::ReflectionPass()
{
...
void Laborator9::ForwardRenderPass()
{
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Material *mat)
{
...
void Laborator9::OnInputUpdate(float deltaTime, int mods)
{
	float speed = 2;
	float sp = 2;
	if (window->KeyHold(GLFW_KEY_LEFT)) { lightPos[0].x -= deltaTime * sp; }
	if (window->KeyHold(GLFW_KEY_RIGHT)) { lightPos[0].x += deltaTime * sp; }
	if (window->KeyHold(GLFW_KEY_UP)) { lightPos[0].z -= deltaTime * sp; }
	if (window->KeyHold(GLFW_KEY_DOWN)) { lightPos[0].z += deltaTime * sp; }
